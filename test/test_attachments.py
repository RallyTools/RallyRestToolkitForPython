...
def test_add_attachment():
    ...
    rally = Rally(server=RALLY, user=RALLY_USER, apikey=API_KEY, workspace=target_workspace, project=target_project)
...
def test_get_attachment():
    ...
    """
    #rally = Rally(server=RALLY, user=RALLY_USER, password=RALLY_PSWD)
    rally = Rally(server=RALLY, user=RALLY_USER, apikey=API_KEY)
...
def test_get_image_binary_attachment():
    ...
    target_project    = 'Anti-Cyclone'
    target_story      = 'US6099'
    target_attachment = 'alpine-snow-glen-plake-quote.png'
    attachment_type   = 'image/png'
    ...
    attachment_specific = rally.getAttachment(story, target_attachment)
    ...
    import subprocess
    process = subprocess.run(['file', clone_file], stdout=subprocess.PIPE, universal_newlines=True)
...
def test_add_tcr_attachment():
    ...
    """
    #rally = Rally(server=RALLY, user=RALLY_USER, password=RALLY_PSWD)
    rally = Rally(server=RALLY, user=RALLY_USER, apikey=API_KEY, workspace=DEFAULT_WORKSPACE, project=DEFAULT_PROJECT)
    wksp = rally.getWorkspace()
...
def x_test_replace_attachments():
    ...
    #    rally = Rally(server=bogus_server, apikey=API_KEY)
