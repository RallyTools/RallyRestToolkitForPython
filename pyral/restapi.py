...
##
    ...
    def _greased(self, item_data):
        ...
        for attr_name in collection_attributes:
            if not isinstance(item_data[attr_name], list):
                continue
            obj_list = []
            for value in item_data[attr_name]:
                # is value like "someentityname/34223214" ?
                if isinstance(value, bytes) and '/' in value \
    ...
    def _buildRequest(self, entity, fetch, query, order, kwargs):
        ...
        elif isinstance(fetch, bytes) and fetch.lower() != 'false':
            self.hydration = "full"
...
##
    ...
    def getSchemaInfo(self, workspace, project=None):
        ...
        if response.content.__class__ == bytes:
            return json.loads(response.content.decode(encoding='utf_8'))['QueryResult']['Results']
        else:
